...
class VehicleController extends Controller
{
    ...
    public function import_sold_copart_csv(Request $request)
    {
        ...
        foreach ($data as $index => $row) {
            ...
            $vinMatches = ($vehicle->vin === $vin);
            $lotMatches = ($vehicle->auction_lot === $lot || $vehicle->purchase_lot === $lot);

            if ($vinMatches && !$lotMatches) {
                $vehicle->auction_lot = $lot;
                $vehicle->save();
                $vehicle_lot_update_count++;
            }
            ...
            VehicleMetas::updateOrCreate(
                ...
                [
                    'meta_value' => Carbon::parse($row[$positions[$requiredColumns['sale_date']]])->format('Y-m-d'),
                ]
            );
            ...
            VehicleMetas::updateOrCreate(
                ...
                [
                    'meta_value' => $row[$positions[$requiredColumns['sale_price']]] == "" ? 0 : $row[$positions[$requiredColumns['sale_price']]], 
                ]
            );
        ...
        $msg = sprintf("%d vehicle(s) updated (%d lot number(s) updated)", $updated_vehicles, $vehicle_lot_update_count);
        Session::flash('success', $msg);
